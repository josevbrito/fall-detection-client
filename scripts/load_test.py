...
TOKENS_PATH = BASE_DIR / os.getenv("TB_TOKENS_FILE", "device_tokens.json")
...
# Host do broker MQTT. No ThingsBoard Cloud o MQTT fica num host diferente do
# HTTP (mqtt.thingsboard.cloud vs thingsboard.cloud), por isso é separado.
# Se não definido, usa TB_HOST (caso local/self-hosted, onde é o mesmo host).
TB_MQTT_HOST = os.getenv("TB_MQTT_HOST", TB_HOST)
TB_MQTT_PORT = int(os.getenv("TB_MQTT_PORT", "1883"))

# MQTT sobre TLS: "true" quando o broker na nuvem expõe MQTTS (porta 8883).
# No modo local/1883 fica "false" (texto puro). Quando ligado, monta um
# contexto TLS padrão que valida o certificado do servidor (ex: Let's Encrypt).
MQTT_TLS = os.getenv("TB_MQTT_TLS", "false").lower() in ("true", "1", "yes")
TLS_CONTEXT = ssl.create_default_context() if MQTT_TLS else None
...
# HTTP do ThingsBoard (para publicar o resumo do experimento no fim).
TB_HTTP_SCHEME = os.getenv("TB_HTTP_SCHEME", "http")
TB_HTTP_PORT = os.getenv("TB_HTTP_PORT", "8090")
if (TB_HTTP_SCHEME, TB_HTTP_PORT) in (("https", "443"), ("http", "80")):
    TB_BASE_URL = f"{TB_HTTP_SCHEME}://{TB_HOST}"
else:
    TB_BASE_URL = f"{TB_HTTP_SCHEME}://{TB_HOST}:{TB_HTTP_PORT}"
TB_TENANT_EMAIL = os.getenv("TB_TENANT_EMAIL", "fall-tenant@thingsboard.org")
TB_TENANT_PASSWORD = os.getenv("TB_TENANT_PASSWORD", "tenant2026")
...
class SensorSimulator:
    ...
    _confirmed_readings: int = field(default=0, init=False)
    ...
    def read(self, force_fall: bool = False) -> dict:
        ...
        if self._fall_state == FallState.IDLE:
            ...
            az = random.gauss(0, 0.05)
            magnitude = math.sqrt(ax**2 + ay**2 + az**2)
            self._confirmed_readings += 1
            # Mantém fall_detected=True por 5 leituras (~500 ms) para que o
            # alarme no ThingsBoard persista e não seja criado/limpo em ciclo
            # rápido — o que gera notificações duplicadas.
            if self._confirmed_readings >= 5:
...
class Metrics:
    ...
    def percentile(self, q: float) -> float:
        """Percentil q (0.0-1.0) das latências. Ex: percentile(0.5) = mediana."""
        if len(self.latencies_ms) < 10:
            return 0.0
        sample = sorted(self.latencies_ms)
        idx = min(int(len(sample) * q), len(sample) - 1)
        return sample[idx]
        ...
        for attempt in range(max_reconnect_attempts):
            try:
                async with aiomqtt.Client(
                    ...
                    tls_context=TLS_CONTEXT,  # None = sem TLS (local); contexto = MQTTS (nuvem)
                ) as client:
                    if attempt == 0:
...
async def main(n_devices: int, n_requests: int, interval_ms: int, offset: int = 0) -> None:
    ...
    console.print(f"  ThingsBoard:      {TB_MQTT_HOST}:{TB_MQTT_PORT} (TLS={MQTT_TLS})")
    console.print()
    ...
    # Publica o resumo no ThingsBoard (device "load-test-stats") para o dashboard
    # ler. Não-fatal: se falhar, o load test continua sendo um sucesso.
    try:
        await publish_summary_to_tb(report)
        console.print("  [green]Resumo publicado no ThingsBoard (load-test-stats).[/green]")
    except Exception as e:
        console.print(f"  [yellow]Não publiquei o resumo no TB: {e}[/yellow]")


if __name__ == "__main__":
    # No Windows, o event loop padrão (Proactor) não suporta add_reader/add_writer
    # que o aiomqtt usa nos sockets → força o SelectorEventLoop.
    import sys
