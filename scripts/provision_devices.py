...
# Arquivo de tokens. Configurável por env para separar nuvem do local
# (ex: TB_TOKENS_FILE=device_tokens.cloud.json no .env da nuvem).
...
# Esquema HTTP: "http" no modo local; "https" quando o ThingsBoard está atrás
# do Caddy na nuvem (TLS automático em iot.josevbrito.com).
TB_HTTP_SCHEME = os.getenv("TB_HTTP_SCHEME", "http")
TB_HTTP_PORT = os.getenv("TB_HTTP_PORT", "8090")
# Omite a porta da URL quando é a padrão do esquema (443/https, 80/http) —
# assim https://iot.josevbrito.com fica limpo, sem ":443".
...
# Modo ThingsBoard Cloud (thingsboard.cloud): você já É o tenant admin, não
# existe login de sysadmin nem criação de tenant. Quando TB_CLOUD=true o script
# loga direto com sua conta do Cloud (TB_TENANT_EMAIL/TB_TENANT_PASSWORD) e vai
# direto para criar o device profile + devices.
TB_CLOUD = os.getenv("TB_CLOUD", "false").lower() in ("true", "1", "yes")
...
async def login_tenant_direct(client: httpx.AsyncClient) -> str:
    """Login direto como tenant admin (usado no ThingsBoard Cloud).

    No Cloud você já é o tenant admin: basta logar com seu e-mail/senha do
    thingsboard.cloud. Não há sysadmin nem impersonação.
    """
    resp = await client.post(
        f"{TB_BASE_URL}/api/auth/login",
...
async def main(n_devices: int, concurrency: int) -> None:
    ...
    async with httpx.AsyncClient(follow_redirects=False) as client:
        if TB_CLOUD:
            # ThingsBoard Cloud: login direto como tenant admin (sua conta).
            console.print("\n[cyan]Autenticando no ThingsBoard Cloud...[/cyan]")
            tenant_token = await login_tenant_direct(client)
            console.print("[green]OK[/green]")
        else:
            # Self-hosted: fluxo sysadmin → cria tenant → impersona.
...
if __name__ == "__main__":
    # Compatibilidade com Windows (SelectorEventLoop).
    import sys
